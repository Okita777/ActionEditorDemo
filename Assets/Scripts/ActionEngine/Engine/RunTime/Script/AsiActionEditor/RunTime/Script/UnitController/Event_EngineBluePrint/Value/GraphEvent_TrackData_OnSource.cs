...
{
    [System.Serializable]
    public class GraphEvent_TrackData_OnSource : BluePrint_Unit
    {
        ...
        public override void Init(ActionStatePart part, ActionMachineTime _time)
        {
            ...
            //    $"的所有者[{part.ActionStateMachine.CurUnit.GetSource.gameObject}({part.ActionStateMachine.CurUnit.GetSource.gameObject.GetHashCode()})]" +
            //    $"\n所有者命中了单位[{part.ActionStateMachine.CurUnit.GetSource.ActionStateMachine.HitUnit is not null}]");
            //            if (!m_UnitVal.IsNode)
            //            {
            //#if UNITY_EDITOR
            //                if (!Valid(part, part)) return;
            //#endif
            //                m_ReturnVal = part.ActionStateMachine.CurUnit.GetSource;
            //            }
            //            else
            //            {
            ...
            //#endif
            //                m_ReturnVal = m_UnitVal.value.ActionStateMachine.CurUnit.GetSource;
            //            }
            //m_ReturnVal = null;
            m_ReturnVal = null;
            UnitVal.Init(part, _time);
            if (UnitVal.isValid(part)) m_ReturnVal = UnitVal.value.GetUnit().GetSource;
        ...
        {
#if UNITY_EDITOR
            if (part.ActionStateMachine.CurUnit.GetSource is null)
            {
...
#if UNITY_EDITOR
        //保存时避免重复实例化，导致引用地址变更
        [System.NonSerialized] protected GraphEvent_TrackData_OnSource GraphEventG = null;
#endif
        ...
        public override BluePrint_Value Clone()
        {
            ...
            {
                GraphEventG = new GraphEvent_TrackData_OnSource();
