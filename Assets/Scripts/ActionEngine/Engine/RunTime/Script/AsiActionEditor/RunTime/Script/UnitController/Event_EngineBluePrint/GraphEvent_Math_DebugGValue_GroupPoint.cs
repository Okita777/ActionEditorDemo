...
namespace AsiActionEngine.RunTime.Graph
{
    [System.Serializable]
    //带参的Vector3
    public class GraphEvent_Math_DebugGValue_GroupPoint : BluePrint_GroupPointData
    {
        [SerializeReference] protected BluePrint_GroupPointData m_ReadValue = new GraphEvent_GValue_GGPoint();
        ...
        [EditorGraphProperty("ReadValue(GroupPoint)", true, EditorGraphPropertyType.EEPT_GGroupPoint, LabelWidth = 150)]
        public BluePrint_GroupPointData ReadValue
        {
        ...
        public override void Init(ActionStatePart part, ActionMachineTime _time)
        {
            ...
            foreach (PointData p in m_ReturnVal)
            {
                EngineDebug.DrawSphere(p.pos, Radius.value, _color, Delay.value);
                EngineDebug.DrawLine(p.pos, p.pos + p.rot * Vector3.forward * Radius.value, _color, Delay.value);
            }
#endif
        }
...
#if UNITY_EDITOR
        //保存时避免重复实例化，导致引用地址变更
        [System.NonSerialized] protected GraphEvent_Math_DebugGValue_GroupPoint GraphEventG = null;
#endif
        ...
        public override BluePrint_Value Clone()
        {
            ...
            {
                GraphEventG = new GraphEvent_Math_DebugGValue_GroupPoint();
                GraphEventG.ReadValue = (BluePrint_GroupPointData)m_ReadValue.Clone();
