...
{
    [System.Serializable]
    public class Event_SetGGroupInt : IActionEventData
    {
        [SerializeField] protected GraphEvent_NoValue_GroupInt m_GraphEvent = new GraphEvent_NoValue_GroupInt();
        [SerializeField] protected GGroupInt m_GGroupVal = new GGroupInt();
        [SerializeField] protected bool m_ExitRun = false;
        [SerializeField] protected GraphEvent_NoValue_GroupInt m_GraphEvent_Exit = new GraphEvent_NoValue_GroupInt();
        ...
        [EditorProperty("设置GV", EditorPropertyType.EEPT_GGroupInt)]
        public GGroupInt GGroupVal
        {
        ...
        public GraphEvent_NoValue_GroupInt GraphEvent_Exit
        {
            get
            {
#if UNITY_EDITOR
                if (m_GraphEvent_Exit is null) m_GraphEvent_Exit = new GraphEvent_NoValue_GroupInt();
#endif
        ...
        public int GetEvenType() => -(int)EEvenTypeInternal.EET_SetGGroupInt;
        [NonSerialized] private ActionEngine_Unit m_Unit;

        public IActionEventData Creact() => new Event_SetGGroupInt();

        public void Enter(ActionStatePart _actionState, bool _isSingle)
        {
            m_Unit = TargetUnit.value(_actionState, EngineResourcesManager.Instance.MachineTime).GetUnit();
            List<int> val = GraphEvent.value(_actionState, new ActionMachineTime(0, 0, 0, 0));
            m_GGroupVal.SetValue(m_Unit.ActionStateMachine.FirstStatePart, val.ToArray());
        }

        public void Update(ActionStatePart _actionState, ActionMachineTime _actionTime)
        {
            List<int> val = GraphEvent.value(_actionState, _actionTime);
            m_GGroupVal.SetValue(m_Unit.ActionStateMachine.FirstStatePart, val.ToArray());
        }
        ...
        public void Exit(ActionStatePart _actionState, bool _interruot)
        {
            if (ExitRun)
            {
                List<int> val = GraphEvent_Exit.value(_actionState, EngineResourcesManager.Instance.MachineTime);
                m_GGroupVal.SetValue(m_Unit.ActionStateMachine.FirstStatePart, val.ToArray());
            }
        }
        ...
        {
            Event_SetGGroupInt eventCast = _eventData as Event_SetGGroupInt;
            eventCast.m_GraphEvent = m_GraphEvent.Clone();
            eventCast.m_GGroupVal = (GGroupInt)m_GGroupVal.Clone();
