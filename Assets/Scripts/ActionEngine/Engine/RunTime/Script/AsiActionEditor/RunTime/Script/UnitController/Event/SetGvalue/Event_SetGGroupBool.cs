...
{
    [System.Serializable]
    public class Event_SetGGroupBool : IActionEventData
    {
        [SerializeField] protected GraphEvent_NoValue_GroupBool m_GraphEvent = new GraphEvent_NoValue_GroupBool();
        [SerializeField] protected GGroupBool m_GGroupVal = new GGroupBool();
        [SerializeField] protected bool m_ExitRun = false;
        [SerializeField] protected GraphEvent_NoValue_GroupBool m_GraphEvent_Exit = new GraphEvent_NoValue_GroupBool();
        ...
        public GraphEvent_NoValue_GroupBool GraphEvent
        {
            get { return m_GraphEvent; }
            set { m_GraphEvent = value; }
        }
        [EditorProperty("设置GV", EditorPropertyType.EEPT_GGroupBool)]
        public GGroupBool GGroupVal
        {
        ...
        public GraphEvent_NoValue_GroupBool GraphEvent_Exit
        {
            get
            {
#if UNITY_EDITOR
                if (m_GraphEvent_Exit is null) m_GraphEvent_Exit = new GraphEvent_NoValue_GroupBool();
#endif
        ...
        public int GetEvenType() => -(int)EEvenTypeInternal.EET_SetGGroupBool;
        [NonSerialized] private ActionEngine_Unit m_Unit;

        public IActionEventData Creact() => new Event_SetGGroupBool();

        public void Enter(ActionStatePart _actionState, bool _isSingle)
        {
            m_Unit = TargetUnit.value(_actionState, EngineResourcesManager.Instance.MachineTime).GetUnit();
            List<bool> val = GraphEvent.value(_actionState, new ActionMachineTime(0, 0, 0, 0));
            m_GGroupVal.SetValue(m_Unit.ActionStateMachine.FirstStatePart, val.ToArray());
        }

        public void Update(ActionStatePart _actionState, ActionMachineTime _actionTime)
        {
            List<bool> val = GraphEvent.value(_actionState, _actionTime);
            m_GGroupVal.SetValue(m_Unit.ActionStateMachine.FirstStatePart, val.ToArray());
        }
        ...
        public void Exit(ActionStatePart _actionState, bool _interruot)
        {
            if (ExitRun)
            {
                List<bool> val = GraphEvent_Exit.value(_actionState, EngineResourcesManager.Instance.MachineTime);
                m_GGroupVal.SetValue(m_Unit.ActionStateMachine.FirstStatePart, val.ToArray());
            }
        }
        ...
        {
            Event_SetGGroupBool eventCast = _eventData as Event_SetGGroupBool;
            eventCast.m_GraphEvent = m_GraphEvent.Clone();
            eventCast.m_GGroupVal = (GGroupBool)m_GGroupVal.Clone();
