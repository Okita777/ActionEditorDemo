...
{
    [System.Serializable]
    public class GraphEvent_TrackData_Attacker : BluePrint_Unit
    {
        ...
        public override void Init(ActionStatePart part, ActionMachineTime _time)
        {
            ...
            //            if (m_UnitVal is null || !m_UnitVal.IsNode)
            //            {
            //#if UNITY_EDITOR
            //                if (!Valid(part, part)) return;
            //#endif
            //                m_ReturnVal = part.ActionStateMachine.AttackerUnit;
            //            }
            //            else
            //            {
            ...
            //#endif
            //                m_ReturnVal = m_UnitVal.value.ActionStateMachine.AttackerUnit;
            //            }

            UnitVal.Init(part, _time);
            if (UnitVal.isValid(part)) m_ReturnVal = UnitVal.value.GetUnit().ActionStateMachine.AttackerUnit;
        ...
        private bool Valid(ActionStatePart part, ActionStatePart self)
        {
            ...
            //if (part.ActionStateMachine.HitUnit is null)
            {
                if (part.ActionStateMachine.AttackerUnit is null)
                {
                    ...
                    {
                        EngineDebug.LogError("出错啦！！！  尝试获取攻击者，但是还没有发生任何攻击行为" +
...
#if UNITY_EDITOR
        //保存时避免重复实例化，导致引用地址变更
        [System.NonSerialized] protected GraphEvent_TrackData_Attacker GraphEventG = null;
#endif
        ...
        public override BluePrint_Value Clone()
        {
            ...
            {
                GraphEventG = new GraphEvent_TrackData_Attacker();
