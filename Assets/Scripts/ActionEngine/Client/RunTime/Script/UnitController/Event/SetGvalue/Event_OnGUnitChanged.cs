...
{
    [System.Serializable]
    public class Event_OnGUnitChanged : IActionEventData, IGValueListenEvent
    {
        ...
        [SerializeField] protected GraphEvent_NoValue_Unit m_BluePrint = new GraphEvent_NoValue_Unit();
        [SerializeField] protected GUnit m_WriteGUnit = new GUnit(true);
        ...
        public GraphEvent_NoValue_Unit BluePrint { get => m_BluePrint; set => m_BluePrint = value; }
        [EditorProperty("写入GUnit", EditorPropertyType.EEPT_Unit)]
        public GUnit WriteTarget { get => m_WriteGUnit; set => m_WriteGUnit = value; }
        ...
        public int GetEvenType() => (int)EEvenType.EET_OnGUnitChanged;
        public IActionEventData Creact() => new Event_OnGUnitChanged();
        ...
        public void Enter(ActionStatePart _actionState, bool _isSingle)
        {
            ...
            void RunWrite()
            {
                ActionEngine_Unit writeTarget = TargetUnit.value(_actionState, EngineResourcesManager.Instance.MachineTime).GetUnit();
                if (writeTarget == null) return;
                AsiActionEngine.RunTime.TargetUnit unitRef = m_BluePrint.value(_actionState, EngineResourcesManager.Instance.MachineTime);
                m_WriteGUnit.SetValue(writeTarget.ActionStateMachine.FirstStatePart, unitRef);
            }
            switch (m_ListenKind)
            {
        ...
        {
            Event_OnGUnitChanged _event = _eventData as Event_OnGUnitChanged;
            ...
            _event.m_WriteGUnit = (GUnit)m_WriteGUnit.Clone();
