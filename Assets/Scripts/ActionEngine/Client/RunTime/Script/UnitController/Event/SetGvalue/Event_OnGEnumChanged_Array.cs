...
{
    // GEnum 底层为 int，数组版使用 GGroupInt 存储一组枚举值
    [System.Serializable]
    public class Event_OnGEnumChanged_Array : IActionEventData, IGValueListenEvent
    {
        [SerializeField] protected EGValueListenKind m_ListenKind = EGValueListenKind.GEnum;
        ...
        [SerializeField] protected GraphEvent_NoValue_GroupInt m_BluePrint = new GraphEvent_NoValue_GroupInt();
        [SerializeField] protected GGroupInt m_WriteGGroupInt = new GGroupInt();
        ...
        public GraphEvent_NoValue_GroupInt BluePrint { get => m_BluePrint; set => m_BluePrint = value; }
        [EditorProperty("写入GGroupInt(GEnum)", EditorPropertyType.EEPT_GGroupInt)]
        public GGroupInt WriteTarget { get => m_WriteGGroupInt; set => m_WriteGGroupInt = value; }
        ...
        public int GetEvenType() => (int)EEvenType.EET_OnGEnumChanged_Array;
        public IActionEventData Creact() => new Event_OnGEnumChanged_Array();
        ...
        public void Enter(ActionStatePart _actionState, bool _isSingle)
        {
            ...
            void RunWrite()
            {
                ...
                int[] val = m_BluePrint.value(_actionState, EngineResourcesManager.Instance.MachineTime).ToArray();
                m_WriteGGroupInt.SetValue(targetUnit.ActionStateMachine.FirstStatePart, val);
            }
            switch (m_ListenKind)
            {
        ...
        public void Exit(ActionStatePart _actionState, bool _interruot)
        {
            ...
            {
                case EGValueListenKind.GInt:
                    if (m_CallbackInt != null) { m_StateMachine.RemoveGIntChanged(m_ListenGInt, m_CallbackInt); m_CallbackInt = null; }
                    break;
                case EGValueListenKind.GFloat:
                    if (m_CallbackFloat != null) { m_StateMachine.RemoveGFloatChanged(m_ListenGFloat, m_CallbackFloat); m_CallbackFloat = null; }
                    break;
                case EGValueListenKind.GEnum:
                    if (m_CallbackEnum != null) { m_StateMachine.RemoveGEnumChanged(m_ListenGEnum, m_CallbackEnum); m_CallbackEnum = null; }
                    break;
                case EGValueListenKind.GBool:
                    if (m_CallbackBool != null) { m_StateMachine.RemoveGBoolChanged(m_ListenGBool, m_CallbackBool); m_CallbackBool = null; }
                    break;
                case EGValueListenKind.GUnit:
                    if (m_CallbackUnit != null) { m_StateMachine.RemoveGUnitChanged(m_ListenGUnit, m_CallbackUnit); m_CallbackUnit = null; }
                    break;
            }
            m_StateMachine = null;
        }
        ...
        {
            Event_OnGEnumChanged_Array _event = _eventData as Event_OnGEnumChanged_Array;
            ...
            _event.m_WriteGGroupInt = (GGroupInt)m_WriteGGroupInt.Clone();
            _event.m_TargetUnit = m_TargetUnit.Clone();
