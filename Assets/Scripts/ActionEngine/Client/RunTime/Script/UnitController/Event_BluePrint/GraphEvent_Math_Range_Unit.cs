...
namespace AsiActionEngine.RunTime.Graph
{
    [System.Serializable]
    //带参的Vector3
    public class GraphEvent_Math_Range_Unit : BluePrint_Unit
    {
        ...
        public override void Init(ActionStatePart part, ActionMachineTime _time)
        {
            ...
            //m_IntVal.SetListSource(EngineResourcesManager.Instance.GetUnits());//提供原始List
            IntVal.Init(part, _time);
            //Debug.LogWarning($"<color=#ffcc00>找到了对象2[{IntVal.value.Count}]</color>");
            ...
            if (m_IntVal.value.Count > 0)
            {
                ...
                if (m_RangeType == 0)
                {
                    ...
                    {
                        m_dir_f = (m_IntVal.value[i].transform.position - TargetPos.value).normalized;
                ...
                else
                {
                    ...
                    for (int i = 0; i < m_IntVal.value.Count; i++)
                    {
                        m_MinValue_f = (m_IntVal.value[i].transform.position - TargetPos.value).sqrMagnitude;
                        if (m_MinValue_f < m_MinValue)
                        {
                            m_Index = i;
                            m_MinValue = m_MinValue_f;
...
#if UNITY_EDITOR
        //保存时避免重复实例化，导致引用地址变更
        [System.NonSerialized] protected GraphEvent_Math_Range_Unit GraphEventG = null;
#endif
        ...
        public override BluePrint_Value Clone()
        {
            ...
            {
                GraphEventG = new GraphEvent_Math_Range_Unit();
