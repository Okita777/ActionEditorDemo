...
namespace AsiActionEngine.RunTime.Graph
{
    [System.Serializable]
    //带参的Vector3
    public class GraphEvent_Math_Range_Transform : BluePrint_Transform
    {
        [SerializeReference] protected BluePrint_GroupTransform m_IntVal = new GraphEvent_GValue_GGTransform();
        ...
        #region Property
        [EditorGraphProperty("GroupTrans", true, EditorGraphPropertyType.EEPT_GGroupTransform, LabelWidth = 60)]
        public BluePrint_GroupTransform IntVal
        {
            get { return m_IntVal; }
            set { m_IntVal = value; }
        }
        ...
        public override void Init(ActionStatePart part, ActionMachineTime _time)
        {
            ...
            //m_IntVal.SetListSource(EngineResourcesManager.Instance.GetTransforms());//提供原始List
            ...
            if (m_IntVal.value.Count > 0)
            {
                ...
                if (m_RangeType == 0)
                {
                    ...
                    {
                        m_dir_f = (m_IntVal.value[i].position - TargetPos.value).normalized;
                ...
                else
                {
                    ...
                    for (int i = 0; i < m_IntVal.value.Count; i++)
                    {
                        m_MinValue_f = (m_IntVal.value[i].position - TargetPos.value).sqrMagnitude;
                        if (m_MinValue_f < m_MinValue)
                        {
                            m_Index = i;
                            m_MinValue = m_MinValue_f;
...
#if UNITY_EDITOR
        //保存时避免重复实例化，导致引用地址变更
        [System.NonSerialized] protected GraphEvent_Math_Range_Transform GraphEventG = null;
#endif
        ...
        public override BluePrint_Value Clone()
        {
            ...
            {
                GraphEventG = new GraphEvent_Math_Range_Transform();
                GraphEventG.IntVal = (BluePrint_GroupTransform)m_IntVal.Clone();
