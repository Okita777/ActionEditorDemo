...
namespace SkillEditor.Editor
{
    public sealed class StateTimelineEditorWindow : EditorWindow
    {
        ...
        private StateInterruptTrackConfig _dragInterruptTrack;
        private StateInterruptTrackConfig _dragInterruptTrackTarget;
        ...
        private void DrawTimelineSurface(Rect outerRect)
        {
            ...
            using (var scroll = new GUI.ScrollViewScope(scrollRect, _timelineScrollPosition, viewRect, false, true))
            {
                ...
                {
                    if (_dragInterruptTrack != null && Event.current.rawType == EventType.MouseUp)
                    {
                        // 必须由本次松手位置重新确认目标，避免在轨道区域外松手时使用上一次悬停目标。
                        _dragInterruptTrackTarget = null;
                    }
        ...
        {
            bool isDragging = ReferenceEquals(_dragInterruptTrack, track);
            bool isSwapTarget = _dragInterruptTrack != null &&
                                !isDragging &&
                                ReferenceEquals(_dragInterruptTrackTarget, track);
            EditorGUI.DrawRect(headerRect,
                isDragging
                    ? new Color(0.16f, 0.42f, 0.50f)
                    : isSwapTarget
                        ? new Color(0.52f, 0.38f, 0.12f)
                        : new Color(0.19f, 0.19f, 0.19f));

            string displayName = GetInterruptTrackDisplayName(track, indexWithinGroup);
            Rect dragHandleRect = new Rect(headerRect.x + 4f, headerRect.y + 5f, 22f, headerRect.height - 10f);
            EditorGUIUtility.AddCursorRect(dragHandleRect, MouseCursor.Pan);
            GUI.Label(dragHandleRect, "≡", _centeredLabel);
            GUI.Label(
                new Rect(headerRect.x + 30f, headerRect.y + 11f, 104f, 20f),
                new GUIContent(displayName, $"P{indexWithinGroup + 1}：数字越小优先级越高；拖拽轨道会自动更新"),
                _trackHeaderLabel);
            ...
            HandleInterruptTrackDrag(dragHandleRect, rowRect, track);
            HandleInterruptTrackContextMenu(rowRect, track);
        }

        private void HandleInterruptTrackDrag(Rect dragHandleRect, Rect rowRect, StateInterruptTrackConfig track)
        {
            Event current = Event.current;
            ...
            if (current.type == EventType.MouseDown && current.button == 0 && dragHandleRect.Contains(current.mousePosition))
            {
                _dragInterruptTrack = track;
                _dragInterruptTrackTarget = null;
                current.Use();
                Repaint();
                return;
            ...
            if (_dragInterruptTrack == null || current.button != 0 || !rowRect.Contains(current.mousePosition))
            {
                return;
            }
            ...
            {
                _dragInterruptTrackTarget = track;
                current.Use();
                Repaint();
                return;
            }
        ...
        private static string GetInterruptTrackDisplayName(StateInterruptTrackConfig track, int indexWithinGroup)
        {
            string trackName = track != null ? track.DisplayName : string.Empty;
            if (string.IsNullOrWhiteSpace(trackName) || IsAutoGeneratedInterruptTrackName(trackName))
            {
                trackName = "打断轨道";
            }

            return $"P{indexWithinGroup + 1} {trackName}";
        }

        private static bool IsAutoGeneratedInterruptTrackName(string displayName)
        {
            const string Prefix = "打断轨道";
            if (string.IsNullOrEmpty(displayName) || !displayName.StartsWith(Prefix, StringComparison.Ordinal))
            {
                return false;
            }
        ...
        private void HandleInterruptTrackDragEnd()
        {
            if (_dragInterruptTrack == null)
            {
                return;
            }

            Event current = Event.current;
            if (current.type == EventType.MouseUp || current.rawType == EventType.MouseUp || current.type == EventType.Ignore)
            {
                if (current.rawType == EventType.MouseUp && _dragInterruptTrackTarget != null)
                {
                    List<StateInterruptTrackConfig> tracks = GetInterruptTracks();
                    int sourceIndex = tracks.IndexOf(_dragInterruptTrack);
                    int targetIndex = tracks.IndexOf(_dragInterruptTrackTarget);
                    if (sourceIndex >= 0 && targetIndex >= 0 && sourceIndex != targetIndex)
                    {
                        StateInterruptTrackConfig targetTrack = tracks[targetIndex];
                        tracks[targetIndex] = _dragInterruptTrack;
                        tracks[sourceIndex] = targetTrack;
                        MarkDirty();
                    }
                }

                _dragInterruptTrack = null;
                _dragInterruptTrackTarget = null;
        ...
        private void ShowAddInterruptConditionMenu(StateInterruptConfig config)
        {
            ...
            AddInterruptConditionMenuItem(menu, config, "角色前向/输入夹角条件", "CompareCharacterForwardToInputInterruptCondition");
